...
MixxxMainWindow::MixxxMainWindow(std::shared_ptr<mixxx::CoreServices> pCoreServices)
        : m_pCoreServices(pCoreServices),
          ...
          m_pPrefDlg(nullptr),
          m_toolTipsMode(mixxx::preferences::TooltipMode::On) {
...
void MixxxMainWindow::initialize() {
    ...
    m_toolTipsMode = pConfig->getValue(
            ConfigKey("[Controls]", "Tooltips"),
            mixxx::preferences::TooltipMode::On);
#ifdef MIXXX_USE_QOPENGL
...
void MixxxMainWindow::slotTooltipModeChanged(mixxx::preferences::TooltipMode tt) {
    m_toolTipsMode = tt;
#ifdef MIXXX_USE_QOPENGL
    ToolTipQOpenGL::singleton().setActive(
            m_toolTipsMode == mixxx::preferences::TooltipMode::On);
...
bool MixxxMainWindow::eventFilter(QObject* obj, QEvent* event) {
    if (event->type() == QEvent::ToolTip) {
        ...
        if (activeWindow &&
            ...
            switch (m_toolTipsMode) {
            case mixxx::preferences::TooltipMode::OnlyInLibrary:
                if (dynamic_cast<WBaseWidget*>(obj) != nullptr) {
                    return true;
                }
                break;
            case mixxx::preferences::TooltipMode::On:
                break;
            case mixxx::preferences::TooltipMode::Off:
                return true;
            default:
                DEBUG_ASSERT(!"m_toolTipsMode value unknown");
                return true;
            }
        }
